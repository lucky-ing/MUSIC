				...
				switch(key)
				{
					...
					case LOW:	//AGC-
								  ...
								  printf("音量：%d\n",recagc);
									...
									break;
				case PLAY:
									if(pname[0]&&rec_sta==0X00)//如果触摸按键被按下,且pname不为空
									{				 
										printf("播放:\n");	   
										printf((u8 *)(pname+11));
										rec_play_wav(pname);						//播放pname  
...
u8 DeleteRecorder(void)
{
	  ...
	  u8 res=0;
	 f_rec=(FIL *)mymalloc(SRAMIN,sizeof(FIL));	//开辟FIL字节的内存区域 
	  if(f_opendir(&recdir,"0:/RECORDER"))//打开录音文件夹
 	 {	 			  
		 myfree(SRAMIN,f_rec);	
		  return 1; 
	 }
	 ...
	 {
		 sprintf((char*)pname,"0:RECORDER/REC%05d.wav",index);
		 res=f_open(f_rec,(const TCHAR*)pname,FA_READ);//尝试打开这个文件
		if(res==FR_OK)
		{
				f_close(f_rec);		//
				res=f_unlink(pname);
